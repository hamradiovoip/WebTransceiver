 ...
 * @version $Revision: 1.10.2.1 $ $Date: 2007/05/14 15:29:42 $
 */
public class Audio8k
    implements AudioInterface, Runnable {

    private final static String version_id =
        "@(#)$Id: Audio8k.java,v 1.10.2.1 2007/05/14 15:29:42 uid1003 Exp $ Copyright Mexuar Technologies Ltd";
    ...
    public Audio8k() {
        ...
        Runnable trec = new Runnable() {
            public void run() { Log.debug("run(): recTick");
                recTick();
            }
        };
        ...
        Runnable tplay = new Runnable() {
            public void run() { Log.debug("run(): playTick");
                playTick();
            }
        };
        _tickp = new Thread(tplay, "Tick-play");
        _tickp.setDaemon(true);
        Log.debug("run(): playTick2");
    ...
    
    {
        while (_tickp != null) {
            try {
                ...
                Thread.sleep(next); //System.out.println("sleep! next");
                Log.verb("Woke");
    ...
    void recTick() {
        ...
        while (_tick != null) {
            ...
            if (delta > 1) { // Only sleep if it is worth it...
                try {
                    Thread.sleep(delta);//System.out.println("sleep! delta= "+delta);
                }
    ...
    private void ringDing() {
        ...
        while (_ringTh != null) {
            ...
            try {
                Thread.sleep(nap);//System.out.println("sleep! nap"+nap);
            }
            catch (InterruptedException ex) {
                ; // who cares
            }
...
// choose one based on audio properties
    ...
    public long readDirect(byte[] buff) throws IOException {
        ...
        while (buffCap <= 0) {
            ...
            try {
                Thread.sleep(20); //System.out.println("sleep! 20");
            }
            catch (InterruptedException ie) {}
    ...
    public void write(byte[] buff, long timestamp) throws IOException {
        ...
        //System.out.println("write! buff len= "+ nbuff.length);
        if (AudioProperties.isStereoRec()) {
        ...
        if (fno < _ofno) {
            _nwrite = fno + 1;
            Log.debug("settting _ofno backwards to " + fno + " (and fixing _nwrite, too)");
        }
        _ofno = fno;
    ...
    private long writeBuff() { 
        ...
        if (!_canWrite) {
            if (top - _nwrite >= (DEPTH + LLBS) / 2) {
                ...
                {
                    // I.e. mic is running
                	Log.debug("mic is running");
                    _micSpeakOffset = _nwrite - _bcount - 2;
                    _micSpeakOffsetValid = true;
                    Log.debug("Set micSpeakOffset " + _micSpeakOffset);
                }
                Log.debug("call startPlay");
                startPlay();
                Log.debug("done startPlay");
                _first = true;
            }
            else 
            {
       ...
       //                     Log.verb("snip - " + start / frameSize +
        ...
        //                }
                        if (diff < -1 * FRAMEINTERVAL) {
                            if (!watergate) _sourceDataLine.write(obuff, 0, frameSize);
                ...
                if (!watergate) _sourceDataLine.write(obuff, start, len);
                played(ab);
                _callLen += 20;
                sample(obuff);
        ...
        Log.debug("ttd for this one is " + ttd);
        return ttd;
    }/* end writeBuff */
  ...
  //      }
    ...
    public static void main(String argv[]) {
        ...
        try {
            ...
            try 
            {            	
                Thread.sleep(20000); //System.out.println("sleep! 20000");
            }
            catch (InterruptedException ex1) {
            }
            a8.test();
...
// really want to get this from Audio.prefs
    ...
    public int getFormatBit() {
        return com.mexuar.corraleta.protocol.VoiceFrame.LIN16_BIT;
    }

    /**
     * run reads from the microphone when available
     */
    public void run() { Log.debug("run():micRead");
        while (_micTh != null) {
            micRead();
        }
    }
