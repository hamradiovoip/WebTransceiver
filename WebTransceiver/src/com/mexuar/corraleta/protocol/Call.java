 ...
 * @version $Revision: 1.43 $ $Date: 2006/11/16 10:55:38 $
 */
/*
...
public class Call
    ...
    private final static String version_id =
        "@(#)$Id: Call.java,v 1.43 2006/11/16 10:55:38 uid100 Exp $ Copyright Mexuar Technologies Ltd";
    ...
    private boolean lastkeyed = false;
    ...
    public Call(Friend source) {
        ...
        Runnable keythread = new Runnable() {
            public void run() { Log.debug("run(): doKeyThread");
                dokeythread();
            }
        };

        Thread _tickp = new Thread(keythread, "Tick-keyed");
        _tickp.setDaemon(true);
        _tickp.setPriority(Thread.NORM_PRIORITY);
        _tickp.start();
    }

    void dokeythread() {
        while(_peer != null) {
            long delta = System.currentTimeMillis() - lastwritems;
            if ((lastkeyed == true) && (delta > 60)) {
                lastkeyed = false;
                _peer.tellGuirxKeyChange(lastkeyed);
            } else if ((lastkeyed == false) && (delta <= 60)) {
                lastkeyed = true;
                _peer.tellGuirxKeyChange(lastkeyed);
            }
            try {
            Thread.sleep(20); //System.out.println("sleep!dokeythread");
            }
...
//        _process.setDaemon(true);
        ...
        Runnable retry = new Runnable() {
            public void run() {Log.debug("run(): resendUnacked ");
                while (!_done) {
                    resendUnacked();
                    try {
                        Thread.sleep(100);//System.out.println("sleep!startRcv");
                    }
...
//        _retry.setDaemon(true);
    ...
    public void audioWrite(byte[] bs, long ts) throws IOException 
    {
    	...
    	//System.out.println("audioWrite called ms="+System.currentTimeMillis());
    	Log.debug("audioWrite called");
        if (_audio != null) 
        {
    ...
    public void sendTEXT(String str) {
        TextFrame txt = new TextFrame(this, str);
    }
    ...
    public void keyradio() {
        ControlFrame rad = new ControlFrame(this);
        rad.sendKeyRadio();
    }
